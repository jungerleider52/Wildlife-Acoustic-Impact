...
def data_parse(filename):
    ...
    with open(filename) as file:
        data = np.loadtxt(file, delimiter=',', dtype=str, usecols=[0,1], skiprows=start_index+1)
        SPL = data[:, 1]
...
def max_curve(maxes, dist, graph=True):
    ...
    """
    # general decaying exponential solution
    def fit_exp(x, a, b, c):
        return a * np.exp(-b * x) + c

    popt = curve_fit(fit_exp, dist, maxes, bounds=(0, [np.inf, np.inf, np.inf]))
    a, b, c = popt[0]
    fitted_values = [fit_exp(d, a, b, c) for d in dist]

    if graph:
        plt.figure()
        plt.scatter(maxes, dist)
        plt.plot(fitted_values, dist)
        plt.show()
...
if __name__ == "__main__":
    ...
    a, b, c = max_curve(antares_max, antares_dist)
    max_curve(d4h_max, d4h_dist)
    max_curve(f9_max, f9_dist)
    max_curve(fh_max, fh_dist)

    print(f"Format: a * exp(-b * x) + c")
    print(f"Where: a = {a:.4f}, b = {b:.4f}, c = {c:.4f}")
